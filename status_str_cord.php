...
$id=$_GET['St_cordinator_id'];
$status=$_GET['Status'];
//$q=" update pharmacist_table set Status = $status where pharmacist_id = $id ";
$q1=" UPDATE store_cordi_table SET Status = '$status' WHERE store_cordi_table.St_cordinator_id = '$id'; ";
$result=mysqli_query($conn,$q1);
 if($result){

//echo '<script type="text/javascript"> alert (" status=true ")</script>';
header('location:strco_signup_signup.php');
}
else {
