...
<form action="" enctype="multipart/form-data" method="post" style="min-width:420px; float:right; margin:2%;background-color: rgba(80, 173, 150, 0.3);border: 1px #e3e8ef;width:30%;border-radius: 5%; text-align: center;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;margin-right: 10%;">
 <div><label   for="quantity" style="font-weight:bold;font-size:20px;color:#000000;margin-bottom:2px;">Drug Quantity:</label>
			  <input value=1 type="number" id="quantity" name="quantity" min="1" style="width:60px;height:30px;  border-radius:5px;"required>
			 <br>  <div>
			<label class="signup"  for="img" style="font-weight:bold;font-size:20px;color:#000000;margin-bottom:2px;">Upload prescription Image:</label>   
			   <input type="file" id="uploadimage" name="uploadimage" style="width:50%;height:30px; border-radius:5px;" required />
...
<?php
  ...
  ?><div style="margin-bottom:50px;"><?php
...
VALUES ('$id','$med_id' ,'$Drug_name', '$Comapany_name', '$Price', '$quantity', '$Type', '$invoice', NOW(),'$folder','$folder1','$Delivery_date','$Delivery_time','$Delivery_options','$Customer_city','$Customer_address','$folder1','yes','$Mobile_num');";
...
// $query_del="delete from customer_order where med_id='$id'";
 $result=mysqli_query($conn,"SELECT * FROM customer_order where Customer_id='$id' AND ( Action='pending' OR Action='Finished' ) ")or die(mysqli_error());
		 ...
		 echo "<table border='' cellpadding='5' align='center' > ";
		  ...
		  echo "<tr style='background:#0082e6;color:white;'><th>prescription_image </th><th>Drug Name</th><th>Quantity</th><th>Comapany Name </th><th>Price </th><th>Type </th><th>Status </th><th>Delete </th></tr>";
		$count=1;
