...
<a href="logout_ph.php" style="background:crimson;"><i class="fa fa-power-off"></i>&nbsp;Logout</a>
</div>
</div>
<label class="mn" style="position:absolute;">&nbsp Manage Pharmacist &nbsp;</label>
...
<div id="content_2" style="margin-right: 22%;margin-top:.5%;" > 
			...
			<form method="post" enctype="multipart/form-data" action=" "  style="margin:2%;background-color: rgba(80, 173, 150, 0.3);border: 1px #e3e8ef;border-radius: 5%; text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;">
			  <div><label for="uname1" style="font-weight: bold; font-size:22px;text-align:center; color: #000000;"> <span>Pharmacist Sign  </span><span>  Up Form</span> </label></div>
			  <!--div  ><br> 
			  ...
			  <div > 
				...
				<input type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" title="Invalid email address"  id="ph_email" name="ph_email" style="width:50%;height:30px; border-radius:5px;" required/>
			  </div><br>
			  <div  >
			  ...
			  <div>
				...
				<input type="number"  id="phone"  name="phone" maxlength="13" minlength="10" style="width:50%;height:30px; border-radius:5px;" required/>
			  </div><br>
			  <div >
    ...
    <option value="Male">Male</option>    
			  ...
			  <div >
				...
				<input type="number" id="age"  min="18" max="200"  name="age" style="width:50%;height:30px;border-radius:5px;" required/>
			  </div><br>
...
$code=rand(99100100,109144144);
$userid=$code."-".$ld;	
	
$Cusid="PHA-";
$PharmacistId=$Cusid.$userid;
...
//$conn=mysqli_connect("localhost","root","","drug");
     ...
     $sql7 = mysqli_query($conn," SELECT * FROM user_table WHERE  ID='$PharmacistId' AND Email='$Email' ")or die(mysql_error());
...
echo '<script type="text/javascript"> alert (" Exit inserted data ")</script>';			
    ...
    // $query=" INSERT INTO pharmacist_table (pharmacist_id, First_name, Middle_name, Last_name, Email, Username, Password, Phone, Sex, Age, Date, Time) 
		//   VALUES ('$PharmacistId', '$First_name', '$Middle_name', '$Last_name', '$Email', '$UserName', '$Password', '$Phone', '$Sex', '$Age', NOW(), NOW())";	
   $query=" INSERT INTO `pharmacist_table` (`pharmacist_id`, `First_name`, `Middle_name`, `Last_name`, `Email`, `Username`, `password`, `Phone`, `Sex`, `Age`, `Date`, `Time`, `Status`) 
   VALUES ('$PharmacistId', '$First_name', '$Middle_name', '$Last_name', '$Email', '$UserName', '$Password', '$Phone', '$Sex', '$Age',  NOW(),  NOW(), '1')";
echo "<script>aler('$query')</script>";
...
<section class="scroll1">
		...
		$count=0;
		 //include_once('connect_db.php');
		 $conn=mysqli_connect("localhost","root","","drug");
		 $result=mysqli_query($conn,"SELECT * FROM pharmacist_table")or die(mysqli_error());
		 echo "<table border='' cellpadding='5' align='center'> ";
		 echo "<tr><th>pharmacist_id</th><th>First_name </th><th>Middle_name </th><th>Last_name </th><th>Email </th><th>Username </th><th>Password </th><th>Phone </th><th>Sex </th><th>Age </th><th>Date </th><th>Time</th><th>UPDATE</th><th>Status</th></tr>";
...
<a href="status.php?pharmacist_id=<?php echo $row['pharmacist_id']?> & Status=0 " style="color:white;background:blue;position:relative;padding:11px;padding-right:28px;" > Activate</a>
 
  <?php 
    
  }else{ ?> 
<a href="status.php?pharmacist_id=<?php echo $row['pharmacist_id']?>  & Status=1 "  style="color:white;background:red;position:relative;padding:12px;" > Diactivate</a>    
	<?php 
	}
	?>
	</td>
