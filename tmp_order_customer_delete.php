...
$id=$_GET['Number'];
$sql=" delete from tmp_order_customer where Number='$id'";
$result=mysqli_query($conn,$sql);
//$rows=mysql_fetch_assoc($result);
//header("location:order_page.php");
if($result){
	echo '<script type="text/javascript"> alert (" YOUR ORDER HAS BEEN DELETED ")</script>';
	header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/help_drug_prescription.php");
}else{
