...
//$Time=$_POST['time'];
     ...
     $sql7 = mysqli_query($conn," SELECT * FROM user_table WHERE  ID='$manager_id' AND Email='$Email' ")or die(mysql_error());
...
echo '<script type="text/javascript"> alert (" Exit inserted data ")</script>';			
 	exit();
	   ...
	   $query=" INSERT INTO manager_table (manager_id, First_name, Middle_name, Last_name, Email, Username, Password, Phone, Sex, Age, Date, Time) 
		VALUES ('$manager_id', '$First_name', '$Middle_name', '$Last_name', '$Email', '$UserName', '$Password', '$Phone', '$Sex', '$Age', NOW(), NOW())";	
   $query1= "INSERT INTO user_table (ID, Email, Username, Password, Phone) VALUES ('$manager_id', '$Email', '$UserName', '$Password', '$Phone')";	
...
document.getElementById("mg_email").value="";
...
<label class="mn" style="position:absolute;">&nbsp Manage Manager &nbsp;</label>
...
<ul class="tabs">  
	...
	<div id="content_2" style="margin-right: 22%;margin-top:.5%;" > 
			...
			<form   action=" " method="post" style="margin:2%;background-color: rgba(80, 173, 150, 0.3);border: 1px #e3e8ef;border-radius: 5%; text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;">
			  <div><label for="formsingup" style="font-weight:bold; font-size:22px;text-align:center; color: #000000;"> <span>Manager Sign  </span><span>  Up Form</span> </label></div>
			   <br>
			  ...
			  <div  > 
				<label class="signup" for="mg_email" style="font-weight: bold;font-size:20px; color: #000000;margin-bottom:2px;">Email:</label>
				<input type="email"   id="mg_email" name="mg_email" style="width:50%;height:30px; border-radius:5px;" required>
			  </div><br>
			  <div  >
    ...
    <option value="Male">Male</option>    
			  ...
			  <div >
				...
				<input type="number"  max="200" min="18" id="age"  name="age" style="width:50%;height:30px;border-radius:5px;" required>
			  </div><br>
...
<form method="post" action="manager_signup.php">
...
<input style="background-color:#0082e640;" type="submit" value="click" -->
		...
		$count=0;
		 //include_once('connect_db.php');
		 $conn=mysqli_connect("localhost","root","","drug");
		 $result=mysqli_query($conn,"SELECT * FROM manager_table")or die(mysqli_error());
		 echo "<table border='' cellpadding='5' align='center'> ";
		 echo "<tr><th>manager_id </th><th>First_name </th><th>Middle_name </th><th>Last_name </th><th>Email </th><th>Username  </th><th>Password  </th><th>Phone </th><th>Sex </th><th>Age </th><th>Date </th><th>Time</th><th>UPDATE</th><th>Status</th></tr>";
      ...
      echo '<td>'.$row['manager_id'].'</td>';		
		...
		echo '<td>'.$row['Time'].'</td>';
		?>
	<td>&nbsp;&nbsp;&nbsp;<a href="update_manager.php?manager_id=<?php echo $row['manager_id']?>"  style="color:white;background:green;position:relative;padding:10px;padding-left:29px;text-align:center;" >EDIT</a></td>
	<td>
	<?php 
	if($row['Status']==1)
	{ ?>
	   	
<a href="status_manager.php?manager_id=<?php echo $row['manager_id']?> & Status=0 " style="color:white;background:blue;position:relative;padding:11px;padding-right:28px;" > Activate</a>
 
  <?php 
    
  }else{ ?> 
<a href="status_manager.php?manager_id=<?php echo $row['manager_id']?>  & Status=1 "  style="color:white;background:red;position:relative;padding:12px;" > Diactivate</a>    
	<?php 
	}
	?>
	</td>
