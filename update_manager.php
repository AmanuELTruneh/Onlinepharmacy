...
<?php 
  /* $conn=mysqli_connect("localhost","root","","drug"); 
   $sql3="select * from pharmacist_table where manager_id ='$user' ";
...
$row=mysqli_fetch_array($query); */
	...
	$sql21="select * from manager_table where manager_id= '$id1' ";
...
.t1{
 ...
 <!--ul class="tabs">  
      ...
      <table border="1">
	    <tr><td align="center"><input name="manager_id " type="text" placeholder="--" value="<?php// include_once('connect_db.php');echo $_GET['manager_id '] ?>" /></td></tr>
   ...
   <div id="content_2" style="margin-right: 22%;margin-top:.5%;" >     
			...
			<form action="" method="post" style="margin:2%;background-color: rgba(80, 173, 150, 0.3);border: 1px #e3e8ef;border-radius: 5%; text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;">
			  <div><label for="uname" style="font-weight: bold; font-size:22px;text-align:center; color: #000000;"> <span>Manager Update  </span><span>  Form</span> </label></div>
			  <div><br> 
				<label class="signup" for="ph_id" style="font-weight: bold;font-size:20px; color: #000000;margin-bottom:2px;">Manager Id:</label>
				<input type="text" readonly  id="email" name="ph_id" style="width:50%;height:30px;border-radius:5px;" value="<?php echo $manager_id; ?>"  required>
			  </div><br>
...
//$PharmacistId=$_POST['manager_id ']; 
$manager_id=$_POST['ph_id'];
...
$Time=$_POST['time'];
   ...
   $sql=" UPDATE manager_table SET  First_name = '$First_name', Middle_name = '$Middle_name',
 Last_name = '$Last_name', Email = '$Email',UserName='$UserName',Password='$Password', Phone = '$Phone',Sex='$Sex', Age = '$Age',Date='$Date',Time='$Time' WHERE manager_id ='$manager_id' ";
...
if($result && $manager_id != ''){
 ...
 ?>
<script><?php echo("location.href = '/Drug-I-S/manager_signup.php'"); ?></script>
