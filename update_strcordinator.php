...
$row=mysqli_fetch_array($query); */
      ...
      $Age='';
       $Date='';
        $Time='';    
		$conn=mysqli_connect("localhost","root","","drug");
    if(isset($_GET['St_cordinator_id'])){ 
       $id1=$_GET['St_cordinator_id'];
	// $id=$_SESSION['pharmacist_id'];
	$sql21="select * from store_cordi_table where St_cordinator_id= '$id1' ";
	$result=mysqli_query($conn,$sql21);
	//if(isset($_SESSION['edit'])){
	if($result){
		$row=mysqli_fetch_array($result);
        $St_cordinator_id=$row['St_cordinator_id'];;		
 ...
 <!--ul class="tabs">  
   ...
   <div id="content_2" style="margin-right: 22%;margin-top:.5%;" >     
			...
			<form action="" method="post" style="margin:2%;background-color: rgba(80, 173, 150, 0.3);border: 1px #e3e8ef;border-radius: 5%; text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;">
			  <div><label for="uname" style="font-weight: bold; font-size:22px;text-align:center; color: #000000;"> <span>Strore Cordinator Update  </span><span>  Form</span> </label></div>
			  <div><br> 
				<label class="signup" for="ph_id" style="font-weight: bold;font-size:20px; color: #000000;margin-bottom:2px;">Store cordinator Id:</label>
				<input type="text" readonly  id="email" name="ph_id" style="width:50%;height:30px;border-radius:5px;" value="<?php echo $St_cordinator_id; ?>"  required>
			  </div><br>
...
//$St_cordinator_id=$_POST['pharmacist_id']; 
$St_cordinator_id=$_POST['ph_id'];
...
$Time=$_POST['time'];
   ...
   $sql=" UPDATE store_cordi_table SET  First_name = '$First_name', Middle_name = '$Middle_name',
 Last_name = '$Last_name', Email = '$Email',UserName='$UserName',Password='$Password', Phone = '$Phone',Sex='$Sex', Age = '$Age',Date='$Date',Time='$Time' WHERE St_cordinator_id ='$St_cordinator_id' ";

$result=mysqli_query($conn,$sql);
if($result && $St_cordinator_id != ''){
	
	echo '<script type="text/javascript"> alert (" Updated Successfully ")</script>';
//header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/admin.php");
//header_remove();
//header('location: ph_signup.php');?>
<script><?php echo("location.href = '/Drug-I-S/strco_signup_signup.php'"); ?></script>
