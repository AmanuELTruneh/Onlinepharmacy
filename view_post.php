...
<?php    
		 ...
		 $result=mysqli_query($conn," SELECT * FROM `manager_post` ORDER BY `time` DESC ")or die(mysqli_error());
		 echo "<table style='overflow:scroll;'  border='1' id='myTable' cellpadding='2' align='center'> ";
		 echo "<tr><th>IMAGE</th><th>DATE </th><th>Time </th><th>POST </th></tr>";
	...
	echo '<td><img style="object-fit:contain;width:100px;height:100px;" src="' .$row['post_file'].'"  /></td>';  
	 
	   echo '<td>'.$row['Date'].'</td>';		
		echo '<td>'.$row['time'].'</td>';		
		echo '<td>'.$row['post_test'].'</td>';
