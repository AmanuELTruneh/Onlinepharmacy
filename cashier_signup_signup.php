...
//$Time=$_POST['time'];
     ...
     $sql7 = mysqli_query($conn," SELECT * FROM user_table WHERE  ID='$cashierId' AND Email='$Email' ")or die(mysql_error());
...
echo '<script type="text/javascript"> alert (" Exit inserted data ")</script>';			
 	exit();
	   ...
	   $query=" INSERT INTO cashier_table (Cashier_id, First_name, Middle_name, Last_name, Email, Username, Password, Phone, Sex, Age, Date, Time) 
		VALUES ('$cashierId', '$First_name', '$Middle_name', '$Last_name', '$Email', '$UserName', '$Password', '$Phone', '$Sex', '$Age', NOW(), NOW())";	
     $query1= "INSERT INTO user_table (ID, Email, Username, Password, Phone) VALUES ('$cashierId', '$Email', '$UserName', '$Password', '$Phone')";		
...
<label class="mn" style="position:absolute;">&nbsp Manage Cashier &nbsp;</label>
...
<ul class="tabs">  
	...
	<div id="content_2" style="margin-right: 22%;margin-top:.5%;" > 
			...
			<form   action=" " method="post" style="margin:2%;background-color: rgba(80, 173, 150, 0.3);border: 1px #e3e8ef;border-radius: 5%; text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;">
			  <div><label for="formsingup" style="font-weight:bold; font-size:22px;text-align:center; color: #000000;"> <span>Cashier Sign  </span><span>  Up Form</span> </label></div>
			   <br>
			  ...
			  <div  > 
				<label class="signup" for="ca_email" style="font-weight: bold;font-size:20px; color: #000000;margin-bottom:2px;">Email:</label>
				<input type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" title="Invalid email address"  id="ca_email" name="ca_email" style="width:50%;height:30px; border-radius:5px;" required>
			  </div><br>
			  <div  >
    ...
    <option value="Male">Male</option>    
			  ...
			  <div >
				...
				<input type="number"  id="age"  name="age" style="width:50%;height:30px;border-radius:5px;" onkeyup="ageValidation()" min="18" max="200" required>
			  </div><br>
...
<input style="background-color:#0082e640;" type="submit" value="click" -->
		...
		$count=0;
		 //include_once('connect_db.php');
		 $conn=mysqli_connect("localhost","root","","drug");
		 $result=mysqli_query($conn,"SELECT * FROM cashier_table")or die(mysqli_error());
		 echo "<table border='' cellpadding='5' align='center'> ";
		 echo "<tr><th>Cashier_id </th><th>First_name </th><th>Middle_name </th><th>Last_name </th><th>Email </th><th>Username  </th><th>Password  </th><th>Phone </th><th>Sex </th><th>Age </th><th>Date </th><th>Time</th><th>UPDATE</th><th>Status</th></tr>";
		...
		?>
	<td>&nbsp;&nbsp;&nbsp;<a href="update_cashier.php?Cashier_id=<?php echo $row['Cashier_id']?>"  style="color:white;background:green;position:relative;padding:10px;padding-left:29px;text-align:center;" >EDIT</a></td>
	<td>
	<?php 
	if($row['Status']==1)
	{ ?>
	   	
<a href="status_cashier.php?Cashier_id=<?php echo $row['Cashier_id']?> & Status=0 " style="color:white;background:blue;position:relative;padding:11px;padding-right:28px;" > Activate</a>
 
  <?php 
    
  }else{ ?> 
<a href="status_cashier.php?Cashier_id=<?php echo $row['Cashier_id']?>  & Status=1 "  style="color:white;background:red;position:relative;padding:12px;" > Diactivate</a>    
	<?php 
	}
	?>
	</td>
