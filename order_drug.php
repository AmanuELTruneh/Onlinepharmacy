...
<i style="justify-content:center;justify-items:center;display: -webkit-box;position: absolute; font-size:20px;color:white;background:#30a4ffd1;height:30px;" class="fa fa-search"></i><input type="text" id="search" placeholder=''  name="search" style="border:1px solid #30a4ffd1;color:black; float:right;width:190px;height:30px; border-radius:5px;" />
</div>
</form>
</div>
<!--div style="display:inline-flex;;justify-content: space-between;">
<a style="background:#1b9bff;float:left;color:white;margin-top:10px;margin-left:2%;font-size:25px;position:relative;border-radius:3px;display: inline-flex;position: relative;padding:2px;" href="order_drug_prescription.php">Click Prescription Drugs</a>
...
<td>&nbsp;&nbsp;&nbsp;<a href="order_page.php?med_id=<?php echo $row['med_id']?>" name="add_to_cart" type="submit"  style="display: -webkit-inline-box;min-width:120px;border-radius:10px;width:120px;color:white;background:#1b9bff; padding:10px;padding-left:29px;text-align:center;" >Add to cart&nbsp; <i style="font-size:20px;" class="fa fa-shopping-cart"></i></a></td>			
    ...
    echo '<td><img style="object-fit:contain;width:150px;height:150px;" src="' .$row['Image'].'"  /></td>';   
	   echo '<td>'.$row['Drug_name'].'</td>';		
		...
		echo '<td>'.$row['Type'].'</td>';?> 
		<!--td><input type="submit" id="add_to_cart" name="add_to_cart" value="Add to cart"/></td-->
<td>&nbsp;&nbsp;&nbsp;<a href="#" name="add_to_cart" type="submit"  style="display: -webkit-inline-box;min-width:120px;border-radius:10px;width:120px;color:white;background:#99a4ad; padding:10px;padding-left:29px;text-align:center;" >Not Available&nbsp; <i style="font-size:20px;" class="fa fa-shopping-cart"></i></a></td>			
...
if(!isset($_POST['search'])){
		 ...
		 //include_once('connect_db.php');
		 $conn=mysqli_connect("localhost","root","","drug");
		 $result=mysqli_query($conn," SELECT * FROM register_drug where Prescription='no' ")or die(mysqli_error());
		 echo "<table style='overflow:scroll;'  border='1' id='myTable' cellpadding='2' align='center'> ";
		 echo "<tr><th>Image</th><th>Drug_name </th><th>Comapany_name </th><th>Description </th><th>Price </th><th>Type </th><th>Buy </th></tr>";
...
if(isset($_POST['add_to_cart'])){
	 echo "  <script>alert('  12 ORDER TRY PLEASE ');<script>";	
//if(isset($_GET['med_id'])){
//	 echo "  <script>alert('  34 ORDER TRY PLEASE ');<script>";	
//$med_id=$_GET['med_id'];}
//$ip_add=getUserIP();
$status="pending";
$invoice_no=mt_rand();
...
$query1=" INSERT INTO customer_order ('Customer_id', 'Drug_name', 'Comapany_name', 'Price', 'Type', 'invoice_no', 'Date') VALUES ('$id', '$Drug_name', '$Comapany_name', '$Price', '$Type', '$invoice_no', 'Now()'); ";
$run=mysqli_query($conn,$query1);
if($run){
 echo " <script>alert('YOUR ORDER HAS BEEB SUCCESSFULLY SUMMITED ');<script>";
}else{
