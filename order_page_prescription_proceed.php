...
{
	$number=$_GET['Number'];
	$query1=" select * from tmp_order_customer where med_id = '$med_id' AND Number='$number' ";
$run1=mysqli_query($conn,$query1);
if($run1)
{
	$row=mysqli_fetch_array($run1);
	$quantity=$row['Quantity'];
	$presc_Image=$row['presc_Image'];
}
}
...
?>
 ...
 $query2=" select * from tmp_order_customer where med_id = '$med_id' AND Number='$number'  ";
...
<form action="" enctype="multipart/form-data" method="post" style="min-width:420px; float:right; margin:2%;background-color: rgba(80, 173, 150, 0.3);border: 1px #e3e8ef;width:30%;border-radius: 5%; text-align: center;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;margin-right: 10%;">
 <div><label   for="quantity" style="font-weight:bold;font-size:20px;color:#000000;margin-bottom:2px;">Drug Quantity:</label>
			  <input type="number" value="<?php echo $quantity ?>"  id="quantity" name="quantity" style="width:60px;padding-left:10px;height:30px; border-radius:5px;"  readonly>
			 <br>
			  </div><br>
			  <fieldset>
   ...
   <fieldset>
	...
	<input type="number" id="Mobile_Number" name="Mobile_Number" placeholder="Mobile Number" style="width:50%;height:30px; border-radius:5px;"  />   
	 </fieldset>
	<br>
	<fieldset>
...
VALUES ('$id','$med_id' ,'$Drug_name', '$Comapany_name', '$Price', '$quantity', '$Type', '$invoice', NOW(),'$presc_Image',' ','$Delivery_date','$Delivery_time','$Delivery_options','$Customer_city','$Customer_address','$folder1','yes','$Mobile_num')";
...
if($run > 0){
 ...
 if($run1 > 0){
	// echo '<script type="text/javascript"> alert (" YOUR ORDER HAS BEEN  Prescription ")</script>';
 }
$query2="UPDATE `tmp_order_customer` SET `Status` = 'Finished' WHERE `tmp_order_customer`.`Number` =$number"; 
$run=mysqli_query($conn,$query2);
...
// $query_del="delete from customer_order where med_id='$id'";
 $result=mysqli_query($conn,"SELECT * FROM customer_order where Customer_id='$id' AND ( Action='pending' OR Action='Finished' )  ")or die(mysqli_error());
		 ...
		 echo "<table border='' cellpadding='5' align='center' > ";
		  ...
		  echo "<tr style='background:#0082e6;color:white;'><th>prescription_image </th><th>Drug Name</th><th>Quantity</th><th>Comapany Name </th><th>Price </th><th>Type </th><th>Delete </th></tr>";
		$count=1;
